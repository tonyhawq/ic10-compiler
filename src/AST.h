...
{
	Assignment(const Token& name, std::shared_ptr<Expr> value) :name(name), value(value) {};
	std::shared_ptr<Expr> value;
	Token name;
...
{
	Binary(std::shared_ptr<Expr> left, const Token& op, std::shared_ptr<Expr> right) :left(left), op(op), right(right) {};
	std::shared_ptr<Expr> left;
	Token op;
	std::shared_ptr<Expr> right;
...
{
	Unary(const Token& op, std::shared_ptr<Expr> right) :op(op), right(right) {};
	Token op;
	std::shared_ptr<Expr> right;
